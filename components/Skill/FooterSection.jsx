...
export default function Footer() {
  return (
    <footer className="w-full bg-gradient-to-b from-[#0B1537] to-[#010B2A] flex justify-center items-center py-20 text-white">
      <div className="bg-[#0E1A3A] bg-opacity-90 rounded-3xl shadow-2xl max-w-6xl w-full mx-4 px-8 py-12 grid grid-cols-1 md:grid-cols-3 gap-10 relative border border-blue-900/30">
        ...
        <div>
          ...
          <nav className="flex flex-col gap-2 text-gray-300 text-base">
            {["About Us", "Services", "Collaborations", "Contact Us"].map((item) => (
              <a
                key={item}
                href="#"
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <nav className="flex flex-col gap-2 text-gray-300 text-base">
              {["Business Consulting", "Invest Process", "IT Services", "Online Training"].map((item) => (
                <a
                  key={item}
                  href="#"
                  className="hover:text-blue-400 transition duration-300"
